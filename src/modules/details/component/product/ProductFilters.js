...
const FilterSection = ({ show, setShow }) => {
  const {
    SearchValueByInput,
    InputFilter: { text, category, company, color },
    AllFilterProduct,
    clearAllFilter,
  ...
  return (
    <div className={`filters-items ${show ? "filterItemActive" : ""}`}>
      {/* search by input */}
      {show && (
        <h1 onClick={() => setShow(!show)} style={{ marginLeft:"250px", padding:"20px" }}>
          close
        </h1>
      )}
        ...
        <form className="searchBrandName">
          ...
          >
            {companyOnlydata.map((curData, index) => {
              return (
                <option key={index} value={curData} name="company">
                  {curData}
                </option>
              );
            })}
